...
def _triton_rope_siso(
    ...
    for row_idx in tl.range(pid, num_tokens, row_block_size):
        ...
        if IS_NEOX_STYLE:
            first_half_offsets = (
                tl.arange(0, pad_n_h)[:, None] * hd
                + tl.arange(0, pad_rope_dim // 2)[None, :]
            )
        else:
            first_half_offsets = tl.arange(0, pad_n_h)[:, None] * hd + (
                2 * tl.arange(0, pad_rope_dim // 2)[None, :]
            )
        ...
        qk_tile_1 = tl.load(
            qk_start_ptr + first_half_offsets, mask=first_mask, other=0
        ).to(sin_row.dtype)
        ...
        qk_tile_2 = tl.load(
            qk_start_ptr + second_half_offsets, mask=second_mask, other=0
        ).to(sin_row.dtype)
